...
?>
        <center>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $temps_laser_run = $_POST["temps_laser_run"];
                ...
                foreach ($athletes_data as $categorie => &$athletes) {
                    foreach ($athletes as &$athlete) {
                        ...
                        if ($athlete['nom'] === $nom_athlete) {
                            $athlete['temps_laser_run'] = $temps_laser_run;

                            // Convertir le temps de laser run du format mm'ss en secondes
                            $temps_laser_run_parts = explode("'", $temps_laser_run);
                            $minutes = intval($temps_laser_run_parts[0]);
                            $secondes = intval($temps_laser_run_parts[1]);
                            $temps_laser_run_en_secondes = ($minutes * 60) + $secondes;

                            // Calculer les points pour le temps de laser run
                            // Vous devez adapter cette partie selon le format de votre laser run et le calcul des points
                            $tmp_lr = $categories[$categorie]['lr'];
                            // Exemple de calcul des points (à adapter selon votre besoin)
                            $points_lr = $base_pts_lr - ($temps_laser_run_en_secondes - intval($tmp_lr));
        ...
        <center>
            <?php $title = 'test'; ?>
            <a class='w3-button' href='download_cat_lr.php?file=<?php echo $fileName; ?>&title=<?php echo $title; ?>&cat=<?php echo $categorie; ?>' target='_blank'>Télécharger cette catégorie</a>
            <table border="1">
                <tr>
                    <th>Nom</th>
