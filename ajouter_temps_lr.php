...
// ✅ Table des distances Laser Run par catégorie
$categories = [
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["temps_laser_run"])) {
    ...
    echo "<script>window.location.href = '?competition=$nom_competition&discipline=$discipline&success=1';</script>";
    exit;
}
...
// ✅ Tableau principal
echo "<form method='post' action='?competition=$nom_competition&discipline=$discipline'>";
foreach ($athletes_data as $categorie => $athletes):
    if (!empty($athletes)):
        $id = md5($categorie);
        ?>
        <div class="categorie-block" id="cat_<?php echo $id; ?>">
            ...
            <center>
                ...
                echo "<a class='w3-button' href='download_cat_lr.php?discipline=$discipline&file=$fileName&title=$title&titledoc=$titledoc&cat=$categorie' target='_blank'>Télécharger cette catégorie</a>";
                ?>
            </center>
            <table style='width: 90%;' border='1' align='center'>
                ...
                <?php foreach ($athletes as $athlete): ?>
                    <tr>
                        ...
                        <td>
                            <input type="text" 
                                   name="temps_laser_run[<?= htmlspecialchars($athlete['nom']) ?>]"
                                   value="<?= htmlspecialchars($athlete['temps_laser_run_brut'] ?? '') ?>"
                                   placeholder="ex : 2'34''50, dns ou dnf">
                        </td>
                    </tr>
                <?php endforeach; ?>
            </table><br>
