...
if (isset($_GET['title']) && isset($_GET['file'])) {
    ...
    echo '<p style="position: fixed; bottom: 20px; width: 100%; text-align: center;">
            © '.date("Y").' CJF Saint-Malo Pentathlon Moderne
          </p>';
    ...
    if (!empty($fileName) && file_exists($fileName)) {
        ...
        if (!empty($data)) {
            ...
            // --- Gestion format aplati ou groupé ---
            $grouped = [];
            ...
            // --- Affichage catégorie par catégorie ---
            foreach ($grouped as $categorie => $athletes) {

                if (empty($athletes)) continue;

                // TRI par points_nat décroissants
                usort($athletes, fn($a,$b) => ($b['points_nat'] ?? 0) <=> ($a['points_nat'] ?? 0));

                // Leader
                $leader_points = $athletes[0]['points_nat'] ?? 0;

                // Calcul des différences formatées m'ss
                foreach ($athletes as &$ath) {

                    $p = $ath['points_nat'] ?? 0;
                    $diff = max(0, $leader_points - $p);

                    $min = floor($diff / 60);
                    $sec = $diff % 60;

                    $ath['diff_formatted'] = sprintf("%d'%02d", $min, $sec);
                }
                unset($ath);
                ...
                foreach ($athletes as $ath) {
                    echo "<tr>
                            ...
                            <td>".htmlspecialchars($ath['temps_natation'] ?? "")."</td>
                            <td>".htmlspecialchars($ath['points_nat'] ?? "")."</td>
                            <td>".htmlspecialchars($ath['diff_formatted'] ?? "")."</td>
                          </tr>";
                }
