...
if (isset($_GET['title']) && isset($_GET['file']) && isset($_GET['cat'])) {

    $titre    = $_GET['title']    ?? "Résultats Natation";
    $titredoc = $_GET['titledoc'] ?? $titre;
    $fileName = $_GET['file']     ?? "";
    $categorie = $_GET['cat']     ?? "";
    $discipline = $_GET['discipline'] ?? 'triathle'; // pas utilisé mais conservé si besoin futur
    ...
    if (!empty($fileName) && file_exists($fileName)) {
        ...
        if (!empty($data)) {
            ...
            if ($isFlat) {
                ...
                $grouped = $data; // déjà groupé par catégorie
            }
            ...
            if (!isset($grouped[$categorie])) {
                ...
                foreach ($athletes as $ath) {
                    ...
                    $points = $ath['points_nat'] ?? 0;
                    $diff_points = max(0, $leader_points - $points);

                    // 1 seconde = 2 points -> diff_seconds = diff_points / 2
                    if ($diff_points > 0 && $leader_points > 0) {
                        $diff_seconds = $diff_points / 2.0;
                        $min = floor($diff_seconds / 60);
                        $sec = floor($diff_seconds % 60);
                        $display_diff = sprintf("%d'%02d", $min, $sec);
                    } else {
                        $display_diff = "0'00";
                    }
                    ...
                    echo "<tr>
                            ...
                            <td>" . htmlspecialchars($ath['temps_natation_brut'] ?? $ath['temps_natation'] ?? '') . "</td>
                            <td>" . htmlspecialchars($ath['points_nat'] ?? '') . "</td>
                            <td>" . $display_diff . "</td>
                          </tr>";
                }
    ...
    header('Content-Type: application/pdf');
    header('Content-Disposition: attachment; filename=\"resultat_natation.pdf\"');
    echo $dompdf->output();
    exit;
}
?>
