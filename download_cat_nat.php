    ...
    if (!empty($fileName) && file_exists($fileName)) {
        ...
        if (!empty($data)) {
            ...
            if (isset($grouped[$categorie])) {
                ...
                // Tri par points Laser Run décroissants
                usort($athletes, fn($a, $b) => ($b['points_lr'] ?? 0) <=> ($a['points_lr'] ?? 0));

                // Détermination du leader
                $leader_points = $athletes[0]['points_lr'] ?? 0;

                // Calcul de la différence avec le leader
                foreach ($athletes as &$athlete) {
                    $athlete['diff_points_leader'] = max(0, $leader_points - ($athlete['points_lr'] ?? 0));
                }
                unset($athlete);
                ...
                foreach ($athletes as $athlete) {
                    $diff = $athlete['diff_points_leader'] ?? 0;
                    if ($diff > 90) {
                        $display_diff = "1'30 (+".($diff - 90).")";
                    } else {
                        $min = floor($diff / 60);
                        $sec = $diff % 60;
                    ...
                    echo "<tr>
                            ...
                            <td>" . htmlspecialchars($athlete['temps_laser_run_brut'] ?? '') . "</td>
